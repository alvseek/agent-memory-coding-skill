...
Fill the [Project Info]([path-to-agent-memory-coding-skill]/plan-templates/high-wizard-plan-template.md#project-info) section only (Project, Date, Agent, Theme). Note the visual reference in the Theme field: e.g., `Theme: ... (visual ref: design.html)`
...
3. **Resume control** here after `/run-qa-test` completes. Proceed to Step 20 (Move Plan to Completed).
