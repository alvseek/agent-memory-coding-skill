...
Read the [Code Quality Analysis Template]([path-to-agent-memory-coding-skill]/plan-templates/code-quality-analysis-template.md) — used as a read-only checklist reference in both modes.
...
Walk through each quality dimension in the [Code Quality Analysis Template]([path-to-agent-memory-coding-skill]/plan-templates/code-quality-analysis-template.md). For each dimension:
...
Present findings to [USER-NAME] using the /wait-options.
