...
It **maps**, it never **builds** — filling the gaps it finds belongs to the two builders: `/build-qa-bench` (the rig) and `/build-qa-test` (the tests).
...
| runbooks · playbook · fixtures | **`/build-qa-test`** | The evergreen assets you *run on* the rig, including the Act + Observe scenarios inside runbooks and the playbook. |
...
Next: /build-qa-bench for rig gaps, /build-qa-test for test gaps, /generate-qa-checklist for a shipped change.
```
...
> This maps what EXISTS and how discoverable it is. Rig gaps → /build-qa-bench; test gaps → /build-qa-test; checklist gaps → /generate-qa-checklist.
...
|---|---|---|---|
| Runbooks | {grade} | /build-qa-test | [qa-runbooks-map.md](runbooks/qa-runbooks-map.md) |
| Playbook | {grade} | /build-qa-test | [qa-playbook-map.md](qa-playbook-map.md) |
| Checklists | {grade} | /generate-qa-checklist | [qa-checklists-map.md](checklists/qa-checklists-map.md) |
| Fixtures | {grade} | /build-qa-test | [qa-fixtures-map.md](fixtures/qa-fixtures-map.md) |
...
**Owner**: {/build-qa-bench | /build-qa-test | /generate-qa-checklist}
...
- **/build-qa-test** — the test builder. Owns runbooks · playbook · fixtures. Bench before tests — a fixture has nothing to build against until the loop runs.
