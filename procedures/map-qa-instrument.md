...
Read-only audit of a project's QA instrument. Scans the **whole project** for existing R/I/A/O materials + the 7 QA artifact categories, grades each on **discoverability**, and writes a set of map docs that link to what already exists in-place (or record "not found"). It **maps**, it never **builds** — filling the gaps it finds is `/build-qa-instrument`'s job.

> **Canonical home.** This skill owns the R/I/A/O loop, the artifact ontology, and the grading rubric below. `/setup-qa-instrument` (the legacy monolith, being split into `/map-qa-instrument` + `/build-qa-instrument`) references *up* to these definitions — do not duplicate them downward.
...
Next: /build-qa-instrument to fill the gaps.
```
...
> This maps what EXISTS and how discoverable it is; /build-qa-instrument fills the gaps.
...
{one-line maturity summary + the top gaps to hand to /build-qa-instrument}
```
...
> Links point to materials in-place; grades reflect discoverability. Fill gaps → /build-qa-instrument.
...
- **/build-qa-instrument** — the write counterpart. Map first (find the gaps), build to fill them. `/build-qa-instrument` reads this map to know what to skip vs. generate.
- **/setup-qa-instrument** — the legacy monolith. Being decomposed into this skill (map) + `/build-qa-instrument` (build); it references these canonical definitions rather than restating them.
- **/integration-test** — consumes the built `qa/` instrument at runtime. The map tells it which R/I/A/O scripts and fixtures exist before a run.
