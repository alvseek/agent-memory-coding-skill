...
It composes the atomic generators the way `/wrap-up` composes `/map-orientation` + `/update-memory` (delegation, never embedded logic), and it is built only now that all four parts exist. The whole-system map survives as a **pick** — and invoking `/generate-docs` **is** that pick, across all lenses.
...
- **A single `/generate-<lens>-docs`** (bare) — the cheap *incremental* path: discover one lens's documented-vs-not inventory and generate one deep-dive at a time (per the /discovery-contract).
...
2. **Resolve the orientation-map path** (`MAP_PATH`) via /map-orientation's Prelude, so each delegated `[M]` run can join documented-status. If no map exists, note it — the maps still generate from disk (per the /discovery-contract).
...
|---|---|---|---|
| 1 | /generate-readme `[M]` | root 7Q README (README lens map) | `docs/README.md` |
| 2 | /generate-architecture-docs `[M]` | architecture-map | `docs/architecture/{project}.md` |
| 3 | /generate-domain-docs `[M]` | domain-context-map | `docs/domain/context-map.md` |
| 4 | /generate-flow-docs `[M]` | flow-journey-map | `docs/flows/journey-map.md` |
...
- **The four atomic generators** — /generate-readme · /generate-architecture-docs · /generate-domain-docs · /generate-flow-docs. `/generate-docs` invokes each one's `[M]` (map) path and nothing else; every lens generator remains fully usable standalone (bare discovery + deep-dive) without this orchestrator.
- **/discovery-contract** — governs each generator's *bare discovery* mode (the cheap incremental path). `/generate-docs` is the complementary *whole-surface* path; it does not run discovery itself.
- **/map-orientation** — supplies `MAP_PATH` (documented-status join) at preflight; indexes the produced maps later (at `/wrap-up` or `--rescan`). `/generate-docs` never runs its write modes.
- **/wrap-up** — the delegation precedent: `/generate-docs` composes the generators the same way `/wrap-up` composes `/map-orientation` + `/update-memory`.
...
2. **Generating deep-dives.** The orchestrator produces *maps only*. Per-unit deep-dives stay with the atomic generators (discovery-first). A "generate everything including deep-dives" batch was explicitly rejected (the `all`-scope + bare mass-generation approaches).
