...
1. **Run core awakening** — invoke the memory core `/awaken-agent [domain]` (Phase 1 identity + Phase 2 **central** memory + report). This loads identity, reasoning, emotional, knowledge, and the latest **central** episodic entry. (The core is **project-blind** — this overlay owns project-context loading, in step 2.)

2. **Load project context + resolve localized home** — apply `/localize-context`'s Localized Home Resolution for the current project (cwd):
...
*(Domain Boundary Awareness: the core teaches generic domain humility (recommend a specialist if you know one). As a coding agent with a fleet, sharpen that — consult `fleet-agents.md` and name the right specialist by name when asked outside your domain.)*
