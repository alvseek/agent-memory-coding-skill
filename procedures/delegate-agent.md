...
```bash
bash "[path-to-agent-memory-coding-skill]/fleet-scripts/ask-agent.sh" "[Name|UUID]" "[structured prompt]" "[theme]" "" "[model]"
```
...
To wrap up a completed session: `bash "[path-to-agent-memory-coding-skill]/fleet-scripts/wrap-up-agent.sh" [UUID]`
