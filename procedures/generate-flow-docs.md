...
- **Decision collection** — if Step 1B resolution is ambiguous, present the candidates by invoking the `/wait-options-coding` command procedure (run the command; its format rules are not in context).
