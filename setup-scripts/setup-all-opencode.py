"""Install the agent-memory-coding-skill OVERLAY procedures as ``opencode`` commands.
...
OpenCode discovers global commands as ``*.md`` files in ``~/.config/opencode/commands/``
(``$XDG_CONFIG_HOME/opencode/commands/`` when ``XDG_CONFIG_HOME`` is set); a file named
``<name>.md`` becomes ``/<name>``. No frontmatter is required — the markdown body is the
prompt template — so the compiled output installs unchanged, exactly as for Claude Code.
...
Cross-platform (mirrors ``setup-all-claude-code.py``): run directly on macOS/Linux, or via the
``.bat`` wrapper on Windows.

Usage:        python setup-scripts/setup-all-opencode.py
Env override: AGENT_MEMORY_TARGET_DIR (default: ~/.config/opencode/commands)
"""
...
# Repo root (this script lives at setup-scripts/setup-all-opencode.py).
_ROOT = Path(__file__).resolve().parents[1]

_MANIFEST_NAME = ".agent-memory-coding-skill-opencode-manifest"
_SIBLING_MANIFEST_NAME = ".agent-memory-opencode-manifest"

# Marks the overlay-path definition line in the global AGENTS.md so re-runs never duplicate it.
# Same UUID the Claude Code installer uses — it is the same definition.
...
def _register_overlay_path(instructions_file: Path, overlay_root: Path) -> str:
    """Define ``[path-to-agent-memory-coding-skill]`` in the global AGENTS.md (idempotent).
    ...
    if not instructions_file.is_file():
        return (
            ...
            "        (it creates AGENTS.md), then re-run this installer."
        )
    if _PATH_DEF_UUID in instructions_file.read_text(encoding="utf-8"):
        return "  [path-to-agent-memory-coding-skill] already registered in AGENTS.md — skipped."
...
def main(argv: list[str] | None = None) -> int:
    target = Path(
        os.environ.get("AGENT_MEMORY_TARGET_DIR") or _config_base() / "commands"
    )

    print("=== Setup agent-memory-coding-skill OVERLAY Slash Commands (OpenCode) ===\n")
    ...
    print(_register_overlay_path(_config_base() / "AGENTS.md", _ROOT))
